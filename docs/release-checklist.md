...
|------|-----|
| 日期 | 2026-08-11（UTC+8） |
| 执行人 | 研发（FEATURE_001 #5 implement） |
| 环境 | 生产 health 抽查 + 本地仓库自动化；**非**全量生产人工双客户端演练 |
| 代码基线 | `dev` 分支当时 HEAD（含 #1–#4 文档与 #3 UI） |
...
| R8 | **待生产/试点人工** | 章程与 `/me/guide` 已覆盖；需持 Key 的 Claude Code 再勾一次 |
| R9 | **待生产/试点人工** | 同上 Cursor |
...
|------|------|
| 自动化层 B | **可通过**（R16/R17） |
| 生产核心路径人工 | **部分**（R1 通过；R8/R9 须补勾） |
| E10 扩大公测签字 | **开放**：补 R8/R9（及建议 R4/R6）+ 运营表（E2 演练、E5 评审、E7 对接人）后，在下方「放行」签字 |
...
放行条件建议：R1、R8、R9、R16 均为通过，且 E2 演练表、E5 评审、E7 对接人已填。
