...
| 环境 | ☐ 生产 `tokenhub.haizhi.com`　☐ 其它：________ |
| 备份文件 | `backups/tokenhub-________.sql.gz`（或异地路径） |
| 备份生成时间 | |
| 演练执行时间 | |
| 执行人 | |
| 复核人 | |
| 结果 | ☐ 通过　☐ 失败（原因：________） |
| 导入/抽查摘要 | （例如表数量、employees 行数抽查） |
| 备注 | |

**首次文档交付时**：若尚未在服务器上执行，保持本表空白，由管理员在首次演练后补填；**扩大公测签字前必须有一行「通过」记录。**
