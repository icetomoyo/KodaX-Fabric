...
**2026-08-14 修订**：0.1.0 收口之后先发 **0.1.1**（回归补丁，无新功能点），再规划 **0.1.2**（单一企业三角色控制台）。超管 / 多租户仍不进本序列。
...
| **0.1.1** | 0.1.0 回归补丁：XFF 白名单、镜像嵌前端、restore 不重跑 bootstrap、Redis 健康、别名/Provider RPM 接到生产 | 无新功能点 | 对照 [KNOWN_ISSUES](KNOWN_ISSUES.md) 001–005 已修 |
| **0.1.2** | 单一企业三角色共用后台壳；开发者申请、队长批本队；官方 Key / 渠只给企业管理员 | 17, 21, 24 | 开发者进不了铺渠；A 队批不了 B 队；项目与池跨团队发不出 |
