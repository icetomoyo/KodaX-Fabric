...
审计员不能创建或修改员工、渠道、凭证、授权、配额和系统设置，也不能读取员工 API Key 明文。概览中的人员排名和错误摘要属于运营统计；调用明细和正文仍受日志授权控制。
...
| 员工 API Key | `th_...`；哈希用于鉴权，加密副本仅供管理员在审计条件下读取 |
| 上游凭证 | 仅管理员维护，加密存储，界面不回显完整 Secret |

### 3.4 员工 API Key 安全规则

1. 员工创建 Key 时依次选择上游渠道、兼容协议并填写名称。
2. Key 只在创建成功结果中展示一次，员工需当场保存。
3. 员工 Key 列表只展示名称、前缀、绑定渠道、协议、状态和时间，不提供明文读取接口。
4. 绑定渠道和协议创建后不可编辑；员工需要调整时删除 Key 后重新创建。
5. 管理员可读取仍可解密的 active 员工 Key；每次读取必须写操作审计，响应包含 `Cache-Control: no-store` 和 `Pragma: no-cache`，且前端不得持久化明文。
6. 审计员无权读取员工 Key 明文。
...
4. 查看他人正文、管理员读取员工 Key、授权变动和渠道管理操作均写入操作审计。
...
| API Key | 渠道选择、协议选择、名称、创建结果一次性明文、Key 列表和删除 |
...
- 创建结果直接显示完整 Key 并提供复制按钮；员工确认已保存后才能关闭，关闭后列表不提供再次读取；
...
| 员工管理 | 搜索、新建、批量导入、编辑、启停、重置密码、读取受托管员工 Key | 无 |
