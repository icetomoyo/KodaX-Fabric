...
| 产品基线 | v0.0.5 |
| 文档版本 | 1.4 |
| 产品状态 | v0.0.5 功能实现与本地验证完成，待部署发布 |
| 编制日期 | 2026-08-07 |
| 适用对象 | 产品、研发、测试、运维、信息安全与合规相关人员 |

本文正文定义截至 v0.0.5 的目标产品规则。各项能力的实际可用性以第 5 节状态标记和第 13 节交付状态为准。

> **修订说明（v0.0.5）**：新增基础工单系统，员工可提交并查看自己的工单，管理员可查询全部工单及详情。回复、指派、附件和状态流转不在本版范围内。
...
TokenHub 面向公司内部员工，不提供对外公开注册、API 转售、充值、余额账户或倍率计费。员工可提交内部注册申请，账号仍须由管理员审核开通；员工只持有 TokenHub 签发的内部 API Key，上游凭证由管理员集中托管。
...
| 自动自助开户 | 员工可提交内部注册申请，但必须经管理员审核通过后才会开通账号 |
...
| 角色 | 页面入口 | 能力范围 |
|---|---|---|
| 管理员 `admin` | `/admin/*` | 员工管理、渠道与凭证管理、模型与授权、配额、全量日志、工单、操作审计和运营概览 |
| 员工 `employee` | `/me/*` | 修改密码、创建和删除自己的 API Key、查看自己的用量与调用记录、提交和查看自己的工单 |
...
- `employee` 不能进入管理端；
- 页面不提供管理端和员工端之间的切换入口；
- 用户从 `employee` 变为 `admin` 时，系统在同一事务内 revoke 其全部 active 调用 Key；该用户再次成为 `employee` 时，已 revoke 的 Key 仍保持失效。
...
|---|---|
| 登录标识 | 手机号唯一 |
| 注册申请字段 | 姓名、部门、手机号；三项均必填 |
| 注册申请 | 员工在登录页提交申请，创建为待审核状态；待审核账号不能登录或调用模型 |
| 审核开通 | 管理员在员工管理查看申请并审核通过；账号激活为 `employee`，初始密码固定为 `Hz@123456` |
| 管理员开户 | 管理员仍可单个创建或批量导入员工，并指定初始密码与角色 |
...
6. 管理员均无员工 Key 列表及明文读取接口，不能查看或复制员工 Key。
...
1. 员工在登录页填写姓名、部门和手机号并提交注册申请。
2. 系统创建手机号唯一的待审核员工记录，待审核状态不能登录。
3. 管理员在“员工管理”查看申请信息并审核通过。
4. 系统激活员工账号；员工使用手机号和初始密码 `Hz@123456` 登录，并完成强制改密。
...
| ID | 优先级 | 需求 | 验收要点 | 状态 |
|---|---|---|---|---|
| F-01 | P0 | 手机号和密码登录 | 凭证错误使用统一安全文案；待审核申请在凭证校验通过后提示等待审核；签发有效会话 | ✅ 当前可用 |
| F-02 | P0 | 首次登录和重置后强制改密 | 未改密用户不能访问业务接口或模型代理 | ✅ 当前可用 |
| F-03 | P0 | 员工单建、批量导入、编辑、启停、重置密码及注册申请审核 | 手机号唯一；员工在登录页提交姓名、部门、手机号，管理员审核通过后以 `Hz@123456` 首次登录并强制改密 | ✅ 当前可用 |
| F-04 | P0 | 严格角色路由与接口隔离 | admin 仅管理端，employee 仅员工端，无跨端入口 | ✅ 当前可用 |
...
| F-31 | P1 | 基础工单系统 | 员工提交并查看自己的工单；管理员分页查询全部工单及详情 | ✅ v0.0.5 已实现 |
...
| 页面 | 管理员能力 |
|---|---|
| 概览 | 查看当日核心指标、渠道聚合状态、Top 员工、供应商分布和最近错误 |
| 员工管理 | 搜索、筛选待审核申请、审核开通、新建、批量导入、编辑、启停、重置密码，以及聚合用量详情与趋势；不展示员工 Key 信息 |
| 上游渠道 | 渠道分组、渠道详情、凭证批量录入、测试、发现、启停、权重、授权和删除 |
| 调用日志 | 全量检索、详情与结构化上下文 |
| 配额策略 | 单一默认日 Token 上限（无多策略/员工覆盖） |
| 工单管理 | 按编号、标题或员工姓名查询全部工单并查看详情 |
| 操作审计 | 检索敏感管理操作 |
| 个人设置 | 修改自己的基本信息与密码 |
...
|---|---|---|---|
| 健康检查 | `/health` | 无 | ✅ 当前可用 |
| 注册、登录与改密 | `POST /api/auth/register`、`/api/auth/login`、`/api/auth/change-password` | 注册与登录无；改密需 Session/JWT | ✅ 当前可用 |
| 员工自助 | `/api/me/*` | employee Session/JWT + 已改密 | ✅ 当前可用 |
| 员工可选渠道 | `GET /api/me/upstream-channels` | employee Session/JWT + 已改密 | ✅ 当前可用 |
| 员工工单 | `/api/me/tickets*` | employee Session/JWT + 已改密，仅本人数据 | ✅ v0.0.5 已实现 |
| 管理接口 | `/api/admin/*` | admin Session/JWT + 已改密 + 路由级权限 | ✅ 当前可用 |
| 管理工单 | `/api/admin/tickets*` | admin Session/JWT + 已改密 | ✅ v0.0.5 已实现 |
...
管理员 / 员工浏览器 ── Session/JWT ─► TokenHub Web / API
                                                    │
                                             PostgreSQL + Redis
```
...
| M6 v0.0.5 | 员工提交与查看个人工单，管理员统一查询工单 | ✅ 功能实现与本地验证完成；待发布 |
| M7 生产加固 | HTTPS、备份、告警、归档、对象存储、SSO 和持续集成 | ⬜ 待排期 |

### 13.2 当前可用能力

**身份与管理**

- 手机号登录、强制改密、员工注册申请审核、员工单建/批量导入、编辑、启停和密码重置；
- 员工按渠道、兼容协议和名称创建 Key，支持本人一次性明文、绑定渠道列表、删除和哈希鉴权；管理端不返回员工 Key 信息；
- 管理端员工用量详情、渠道分层详情、结构化日志上下文、单一配额、概览和操作审计页面。
- 员工提交与查看自己的工单，管理员分页查询全部工单及详情。
...
验证日期为 2026-08-07，代码基线为当前 v0.0.5 开发工作区。本节的“当前可用”表示核心开发路径已实现并通过所列验证，不表示已经完成部署发布或生产加固。

| 验证项 | 结果 |
|---|---|
| 服务端默认单元测试 | 包含时区日界线、零额度硬限、用量强一致、两协议上下文解析、敏感字段移除和管理员正文权限守卫 |
| Workspace 生产构建 | Server 与 Web 构建通过 |
| Migration | 干净数据库基线为 `0000_melodic_mattie_franklin.sql`；v0.0.5 新增 `0001_aspiring_mulholland_black.sql` 创建工单表和索引 |
| v0.0.5 工单测试 | 工单编号、员工数据隔离、管理端查询字段和角色路由注册测试通过 |
