...
>
> 1. **审计正文可见性收紧**：管理端调用日志正文改为**仅管理员可见**，审计员不再依据 `can_read_body` 读取正文（`can_read_body` 字段弃用但保留）；员工查看自己正文不变。涉及 §3.1、§4.5、§6.2、§9.3、F-25、F-26。
...
| 审计员 `auditor` | `/admin/*` 的只读子集 | 查看全局运营概览、上游渠道只读信息，以及授权范围内的调用记录**元数据**；**（v0.0.3）正文不再对审计员开放，`can_read_body` 弃用** |
...
1. 员工查看自己的调用列表和审计正文。
2. 管理员检索全量调用记录。
3. 审计员按日志授权范围查看调用元数据；**（v0.0.3）审计员不可查看正文，正文仅管理员可见**（`can_read_body` 弃用保留）。
...
| F-26 | P0 | 日志查阅授权 | 支持 all、dept、employees 范围、有效期和撤销；**（v0.0.3）正文授权 `can_read_body` 弃用**，授权仅控制元数据范围 | ✅ v0.0.3 已实现 |
...
配额以员工为主体执行，不提供多策略、员工覆盖、日请求上限或日 Token 软上限。用量以终态审计累计为准，并发在途可能产生少量超额；超额后后续请求拒绝。配额只用于资源治理，不生成金额或账单。历史策略、覆盖和 `soft_limit_hit` 数据保留用于审计与回滚，但不参与运行时执行。
...
| 审计员 | 日志授权范围 | **（v0.0.3）不可见**；`can_read_body` 弃用保留 |
...
| `quota_policies` | 单一生效的默认日 Token 配额；历史非默认策略保留但不参与执行 |
| `employee_quota_overrides` | v0.0.3 起弃用，仅保留历史数据与回滚能力 |
| `usage_counters_daily` | 员工日请求与 Token 累计 |
| `request_audits` | 调用审计元数据 |
| `request_audit_bodies` | 调用请求与响应正文 |
| `log_access_grants` | 日志元数据范围和有效期授权；`can_read_body` 为弃用兼容字段 |
...
| Migration | PostgreSQL 16 实际执行至 `0002_needy_ultimates.sql`；默认配额唯一、旧软限/请求限清空、null 硬限映射 5 亿、员工日索引存在，历史覆盖表保留 |
| v0.0.3 API/数据库集成 | auditor 用量/context 均 403；用量 counters 汇总强一致、缺失 usage 计数正确；context 去除 Header/Secret；同一管理员 5 分钟重复读取仅一条 `log.read_context`；旧 includeBody 不返回正文 |
