...
| 审计员 `auditor` | `/admin/*` 的只读子集 | 查看全局运营概览、上游渠道只读信息，以及授权范围内的调用记录；正文受 `can_read_body` 控制 |
| 员工 `employee` | `/me/*` | 修改密码、创建和删除自己的 API Key、查看自己的用量与调用记录 |
| 调用方 | `/v1/*` | 使用有效的员工 API Key 调用与查询模型，不具有页面权限 |

审计员不能创建或修改员工、渠道、凭证、授权、配额和系统设置。管理员和审计员均不能列出员工 API Key 或读取其明文；概览中的人员排名和错误摘要属于运营统计，调用明细和正文仍受日志授权控制。
...
5. 管理员员工列表只展示每位员工的 active Key 数量 `activeApiKeyCount`，不返回 Key 列表、前缀、绑定信息或明文。
...
1. 员工查看自己的调用列表和审计正文。
2. 管理员检索全量调用记录。
3. 审计员按日志授权范围查看调用元数据；只有 `can_read_body = true` 时可查看正文。
...
| F-05 | P0 | 员工 Key 基础生命周期 | 支持员工创建、向本人一次性展示明文、摘要列表、删除和哈希鉴权；管理端只返回 `activeApiKeyCount` | ✅ 当前可用 |
...
| ID | 优先级 | 需求 | 验收要点 | 状态 |
|---|---|---|---|---|
| F-22 | P0 | 配额执行 | 支持 RPM、并发、日 Token 与日请求软硬上限 | ✅ 当前可用 |
| F-23 | P0 | 调用审计 | 通过 API Key 鉴权后的成功、参数错误、上游失败、取消和配额限流均记录元数据、正文、usage、终态与重试轨迹 | ✅ 当前可用 |
| F-24 | P0 | 员工用量与日志 | 员工只能查看自己的列表、详情和正文 | ✅ 当前可用 |
| F-25 | P0 | 管理日志检索 | 按员工、时间、模型和状态查询；正文按权限加载并写操作审计 | ✅ 当前可用 |
| F-26 | P0 | 日志查阅授权 | 支持 all、dept、employees 范围、正文权限、有效期和撤销 | ✅ 当前可用 |
| F-27 | P1 | 运营概览 | 提供当日请求、Token、失败数、Top 员工、供应商分布和最近错误快照 | ✅ 当前可用 |
| F-28 | P1 | 配额管理 | 管理默认策略、策略规则和员工覆盖 | ✅ 当前可用 |
...
员工管理列表只返回每位员工的 `activeApiKeyCount`。服务端不提供管理员列出员工 Key 或读取员工 Key 明文的接口，管理端不能查看或复制员工 Key。
...
| 审计员 | 日志授权范围 | 需同时满足授权范围和 `can_read_body` |
...
- 管理员只能通过 `activeApiKeyCount` 查看员工 active Key 数量，不能列出或读取 Key；
...
- 员工明文仅在创建时向本人展示一次，管理员只能查看 `activeApiKeyCount`，不能列出、查看或复制员工 Key；
...
4. 员工明文只在创建响应中向本人出现一次；员工无明文找回接口；管理员只能看到 `activeApiKeyCount`，且员工 Key 列表与明文读取接口均不可达。
