...
本文正文定义 v0.0.1 的目标产品规则。各项能力的实际可用性以第 5 节状态标记和第 13 节交付状态为准。
...
| ID | 优先级 | 需求 | 验收要点 | 状态 |
|---|---|---|---|---|
| F-22 | P0 | 配额执行 | **（v0.0.3 修订）**按员工的单日总 Token 硬上限；RPM/并发改为系统级固定 safeguard，不作业务策略 | 🔄 v0.0.3 收敛 |
| F-23 | P0 | 调用审计 | 通过 API Key 鉴权后的成功、参数错误、上游失败、取消和配额限流均记录元数据、正文、usage、终态与重试轨迹 | ✅ 当前可用 |
| F-24 | P0 | 员工用量与日志 | 员工只能查看自己的列表、详情和正文 | ✅ 当前可用 |
| F-25 | P0 | 管理日志检索 | 按员工、时间、模型和状态查询；**（v0.0.3）正文仅管理员可读**并写操作审计 | 🔄 v0.0.3 修订 |
| F-26 | P0 | 日志查阅授权 | 支持 all、dept、employees 范围、有效期和撤销；**（v0.0.3）正文授权 `can_read_body` 弃用**，授权仅控制元数据范围 | 🔄 v0.0.3 修订 |
| F-27 | P1 | 运营概览 | 提供当日请求、Token、失败数、Top 员工、供应商分布和最近错误快照 | ✅ 当前可用 |
| F-28 | P1 | 配额管理 | **（v0.0.3 修订）**单一默认日 Token 上限策略；移除多策略与员工覆盖 | 🔄 v0.0.3 收敛 |
...
| 员工管理 | 搜索、新建、批量导入、编辑、启停、重置密码、查看 active Key 数量 | 无 |
| 上游渠道 | 渠道分组、凭证批量录入、测试、发现、启停、权重、授权和删除 | 只读元数据 |
...
| 策略 | 行为 |
|---|---|
| RPM | 按员工限制每分钟请求数，超限拒绝 |
| 并发 | 按员工维护 Redis 并发租约，超限拒绝 |
| 日 Token 软上限 | 超限后继续放行并记录告警标记 |
| 日 Token 硬上限 | 启用后超限拒绝 |
| 日请求软上限 | 超限后继续放行并记录告警标记 |
| 日请求硬上限 | 启用后超限拒绝 |

配额以员工为主体执行，可使用默认策略、指定策略和员工直接覆盖。员工直接覆盖支持日 Token 软硬上限、RPM 和并发；日请求软硬上限来自所选策略或默认策略。配额只用于资源治理，不生成金额或账单。
...
| `quota_policies` | 默认或命名配额策略 |
| `employee_quota_overrides` | 员工配额覆盖 |
| `usage_counters_daily` | 员工日请求与 Token 累计 |
| `request_audits` | 调用审计元数据 |
| `request_audit_bodies` | 调用请求与响应正文 |
| `log_access_grants` | 日志范围、正文权限和有效期授权 |
...
- 员工按渠道、兼容协议和名称创建 Key，支持本人一次性明文、绑定渠道列表、删除和哈希鉴权；管理端只展示 active Key 数量；
...
- Redis RPM、并发租约、日 Token 与日请求软硬上限；
...
| v0.0.1 API/数据库集成 | 严格角色、渠道可见性与防泄漏、创建校验、本人一次性明文、管理端仅 active Key 计数、管理员 Key 列表/reveal 路由不可达、NOT NULL/FK、角色吊销全部通过 |
...
| 审计正文包含敏感数据 | 数据泄露 | 安全脱敏、权限分级、正文授权、操作审计、数据库与备份访问控制 |
