...
function EndpointDocView({ doc, origin }: { doc: EndpointDoc; origin: string }) {
  ...
  const gateway = Boolean(doc.fabricHeaders?.length);
    ...
    <article>
      ...
      <div className="grid items-start gap-8 xl:grid-cols-[minmax(0,1fr)_minmax(22rem,28rem)]">
        <div className="min-w-0 space-y-8">
          ...
          {doc.fabricHeaders && doc.fabricHeaders.length > 0 ? (
            <Section title="Fabric 响应头">
              <FieldList fields={doc.fabricHeaders} />
            </Section>
          ) : null}
        </div>
    ...
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm leading-6 text-muted-foreground">
          网关先校验虚拟钥匙，再选渠、打上游。下面是网关自己返回的错误；上游 4xx/5xx
          正文会按协议原样回传。 OpenAI 端点用 <code className="font-mono text-xs">error.code</code>
          ，Anthropic 端点用 <code className="font-mono text-xs">error.type</code>。
        </p>
      </div>
      ...
      <section className="rounded-lg border border-border bg-card p-5">
        ...
        <ul className="mt-3 list-disc space-y-2 pl-5 text-sm leading-6 text-foreground/80">
          <li>
            ...
            ，Header <code className="font-mono text-xs">Authorization: Bearer fab-…</code>
          </li>
          <li>
            Anthropic SDK / Claude Code：Base URL 填{" "}
            <code className="rounded bg-muted px-1.5 py-0.5 font-mono text-xs">{origin}</code>
            ，Header <code className="font-mono text-xs">x-api-key: fab-…</code>
          </li>
          <li>同一把 VK 可打两个端点；官方 Key 不会出现在调用方请求里。</li>
          <li>
            上游限流头（
            <code className="font-mono text-xs">Retry-After</code>、各类{" "}
            <code className="font-mono text-xs">x-ratelimit-*</code>）会回传给调用方。
          </li>
        </ul>
      </section>
        ...
        {fabricErrors.map((err) => (
          ...
          >
            ...
            <div className="mt-3 grid gap-3 lg:grid-cols-2">
              <MiniCode label="OpenAI 形态" value={prettyJson(err.openai)} />
              <MiniCode label="Anthropic 形态" value={prettyJson(err.anthropic)} />
            </div>
          </article>
        ))}
      </div>
