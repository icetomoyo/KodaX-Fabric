...
describe("admin API docs", () => {
  ...
  it("lists every backend API under categories", () => {
    renderDocs();
    const apiCount = navGroups.reduce(
      (n, g) => n + g.items.filter((i) => i.kind === "api").length,
      0,
    );
    expect(allEndpoints).toHaveLength(33);
    expect(apiCount).toBe(33);
    for (const title of [
      "模型 API",
      "系统",
      "账号",
  ...
  it("opens a console API from the list", async () => {
    ...
    expect(screen.getByText("/console/v1/login")).toBeInTheDocument();
    expect(screen.getByText("phone", { selector: "span" })).toBeInTheDocument();
  });
  ...
  it("opens errors from a deep link", () => {
    ...
    expect(screen.getByRole("heading", { name: "鉴权与错误" })).toBeInTheDocument();
    expect(screen.getByText("model_not_allowed")).toBeInTheDocument();
  });
