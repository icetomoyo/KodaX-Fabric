...
```sh
go run ./cmd/gateway
# 进程默认 LISTEN_ADDR=:8080
# compose 对外：API http://127.0.0.1:3000  操作台 http://127.0.0.1:8080
# GET http://127.0.0.1:3000/health
```

操作台（shadcn，网关同进程）：`/` 首页，`/admin` 管理后台，`/me` 开发者申请 VK。  
Cursor / Claude Code 的 Base URL 填 API Origin（本机 `http://127.0.0.1:3000`），同一把 `fab-` 钥匙两个端点都能用。
