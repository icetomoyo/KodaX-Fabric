...
test("key binding page is a full canvas with a filter drawer and unbound-key entry", () => {
  ...
  assert.match(view, /label="组织"/);
  assert.match(view, /企业 \/ 部门 \/ 子部门/);
  assert.match(view, /checkStrictly: true/);
  assert.match(view, /layoutGraph\(item.graph, "enterprise", item.depth\)/);
  assert.match(view, /graphForOrgSelection/);
  assert.match(view, /orgFilterPath/);
  assert.doesNotMatch(view, /filterEnterpriseKey/);
...
test("API Key page auto-provisions per department and protocol, plaintext shown", () => {
  ...
  assert.match(keysView, /新增 Key/);
  assert.match(keysView, /createKey/);
  assert.match(keysView, /showKeys/);
  assert.match(keysView, /displayKey/);
  assert.doesNotMatch(keysView, /创建 Key/);
  assert.doesNotMatch(keysView, /departmentChoices/);
  assert.match(keysView, /showKeys\.value && row\.key/);
  assert.match(keysView, /keyPrefix\}••••/);
  assert.match(keysView, /copyKey/);
  assert.match(keysView, /copyBaseUrl/);
  assert.match(keysView, /copyProtocolUrl/);
  assert.match(keysView, />API协议</);
  assert.match(keysView, /Messages API/);
  assert.match(keysView, /Chat Completions API/);
  assert.match(keysView, /Responses API/);
  assert.match(keysView, /\/v1\/messages/);
  assert.match(keysView, /\/v1\/chat\/completions/);
  assert.match(keysView, /\/v1\/responses/);
  assert.match(keysView, /https:\/\/tokenhub\.haizhi\.com/);
  assert.doesNotMatch(keysView, /window\.location\.origin/);
  assert.doesNotMatch(keysView, /RELAY_BASE_PATH/);
