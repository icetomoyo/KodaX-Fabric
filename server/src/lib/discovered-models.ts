...
export type EmployeeCatalogModel = {
  model: string;
  priced: boolean;
...
export type EmployeeCatalogChannel = Omit<ChannelModelGroup, "models"> & {
  models: EmployeeCatalogModel[];
};

export function attachPricesToChannelModels(
  channels: readonly ChannelModelGroup[],
  prices: readonly ModelPriceAmounts[],
): EmployeeCatalogChannel[] {
  const priceByModel = new Map(prices.map((row) => [row.model, row]));
  return channels.map((channel) => ({
    ...
    models: channel.models.map((model) => {
      ...
      return {
        ...
        promptPricePerMillion: price?.promptPricePerMillion ?? null,
        completionPricePerMillion: price?.completionPricePerMillion ?? null,
        cacheHitPricePerMillion: price?.cacheHitPricePerMillion ?? null,
      };
    }),
  }));
}
