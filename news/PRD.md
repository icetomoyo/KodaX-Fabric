# KodaX Fabric — 可落地版 PRD（分步实施）

> 本文档是 `docs/PRD.md` 的落地重构版：范围不变（Token Hub + Token ROI），但按 **5 个可独立交付的阶段** 重新组织，每个阶段都有明确的验收标准和"不做清单"。
> 同时修正了原 PRD 中阻塞实现的三个问题：Run 界定缺失、权重边界矛盾、V1 技术栈过重。
>
...
| #   | 原 PRD 问题                                                   | 本版修正                                                                                                                                     |
| --- | ---------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | Run 界定缺失（`fabric_context` 无 run_id，重试检测/预算预占无法实现）          | 明确 `x-fabric-context` 增加可选 `run_id`；无 run_id 时按 `VK + 30 分钟滑动窗口` 启发式聚合，并标记 `run_source: inferred`                                        |
| 2   | weight=0.3 边界矛盾（System Prompt/Thinking 既算"有价值"又要"计入低价值比例"） | Token 三分类：**核心价值**（weight > 0.3）/ **必要开销**（System Prompt、Thinking，单独归类）/ **低价值**（weight < 0.3）。Token ROI = 核心价值成本 / 总成本；必要开销单独展示，不混入任何一侧 |
| 3   | V1 技术栈过重（PostgreSQL + Redis + ClickHouse + S3 + K8s 四存储起步） | Phase 1-2 只用 **PostgreSQL + Redis**，单进程 Go 二进制 + Docker Compose 部署。ClickHouse 推迟到 Phase 3（分析查询变慢时才引入），S3 推迟到需要保存正文时                      |
| 4   | Runtime 报告 API 缺失（Outcome ROI 无数据入口）                       | Phase 5 定义 `POST /api/v1/reports/*` 契约；之前阶段不依赖 Runtime                                                                                   |
| 5   | 独立部署认证未定义                                                  | Phase 1 控制台使用本地账号（bcrypt）；Phase 4 增加 OIDC 对接（可选）                                                                                         |
...
核心问题：**企业的 Token 花在哪里？花得值不值？怎么花得更值？**

不可动摇的设计原则（继承原 PRD）：

1. **零转换透传**：OpenAI 端点只路由 OpenAI 系，Anthropic 端点只路由 Anthropic 系，不做跨协议转换
2. **不调用 LLM**：所有优化用统计方法，硬规则由代码执行
3. **凭据不下发**：调用方持 Virtual Key，Provider Key 加密存储、永不外泄
4. **账本 append-only**：不改历史流水，差额用 Adjustment 追加
...
> 开发模式：**单人 + AI coding**。预估按此模式给出——编码不是瓶颈，瓶颈是验证正确性和真实流量磨合，预估中已包含验证时间。
...
判断优先级的标准：Phase 1-2 解决"省心"（对标 LiteLLM/One API，是入场券）；Phase 3 是护城河（竞品没有）；Phase 4 只在确定对外卖 Token 时启动。
...
| Run 关联       | `x-fabric-context` 头支持 `{project_id, task_type, run_id}`；无 run_id 时按 VK + 30 分钟窗口推断聚合                                |
| 管理台（最小）      | 本地账号登录；页面仅 4 个：Provider/Key 管理、VK 管理、价格表、用量报表（按项目/模型/日聚合）                                                            |
...
- 存储：PostgreSQL（配置 + 计量流水）+ Redis（可选，Phase 1 允许不部署）
...
| 渠道健康与熔断      | 滑动窗口成功率（最近 100 次），< 80% 自动禁用；禁用后每 30s 探测，成功恢复。**熔断参数全局统一，UI 文案与此对齐**                                                                     |
| 限流           | 四维令牌桶（per-key / per-project / per-team / per-provider），Redis Lua 原子操作；V1 策略只做**硬拒绝 429**，排队/降级推迟                                         |
| 预算           | 三级 Budget Waterfall（项目 → 团队 → 企业）；软限制 80% 告警（响应头 + Webhook），硬限制 100% 拒绝（402）。**Phase 2 用"事后扣减 + 阈值检查"，预算预占推迟到 Phase 4**（预占主要服务于对外计费的精确性） |
| 组织结构         | Organization → Team → Project → VK 四级；角色：企业管理员 / 团队管理员 / 开发者（超管推迟到 SaaS 需要时）                                                             |
| VK 审批流       | 开发者自服务申请 → 团队管理员审批 → 生成 VK；项目名创建后不可改                                                                                                     |
...
- 数据保存策略：先做 **Metadata Only / Full 两档** + TTL，五档完整版（Redacted/Zero/Local Only + Legal Hold）推迟到有合规客户需求时
...
| 套餐与三层计量        | 订阅额度内 → 超额区（标准价 × markup）→ 透传区（标准价现金）；透传区硬上限 = 月套餐费 3 倍，触发熔断     |
| 预算预占           | 两阶段：P90 初始预占 + 增量追加；Run 结束释放（此时引入，服务于计费精确性）                      |
...
单人 + AI 模式下，工期不再由"代码量 ÷ 人数"决定。真正花时间的是三类无法靠 AI 生成速度压缩的工作，预估围绕它们给出：

1. **正确性验证**：透传语义（各 Provider 的 SSE/usage 格式差异、流式中断、工具调用边界）、限流原子性、账能不能对上——这些必须写测试、跑真实流量
...
3. **单进程 + Compose 贯穿始终**：一个人运维不起微服务，第 9 节第 8 条从"Phase 1-3 不拆分"强化为"出现真实负载瓶颈前永不拆分"
